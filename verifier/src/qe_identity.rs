...
impl SignedQeIdentity {
    ...
    ///
    /// # Arguments
    /// - `key` - The public key to verify the `enclaveIdentity` signature
...
impl<E: Accessor<SignedQeIdentity>> Verifier<E> for SignedQeIdentityVerifier {
    ...
    fn verify(&self, evidence: &E) -> VerificationOutput<Self::Value> {
        ...
        let result = signed_qe_identity.verify(&self.key, self.time);
...
mod test {
    ...
    #[parameterized(
        at_issue_date = {"2023-06-14T15:55:15Z"},
        one_secend_after_issue_date = {"2023-06-14T15:55:16Z"},
        one_second_before_next_update = {"2023-07-14T15:55:14Z"},
    )]
